...
\begin{frame}
    ...
    for all norms. \todo{check statement}
\end{frame}
...
\begin{frame}
    ...
    \todo{check this maths}
    \begin{align*}
        0 = \nabla f(x^{\star}) -  \nabla A^{T}v^{\star} \\
        \nabla f(x^{\star}) = \nabla A^{T}v^{\star} \\
        \nabla f(x^{\star}) = A^{T} v^{\star}
    \end{align*}
    \todo{convergence of newton's method}
\end{frame}

\begin{frame}
    \todo{add linearity of A}
    ...
    \todo{what the hessian needs to be}
\end{frame}
...
\begin{frame}
    ...
    \todo{convergence of the barrier method and choice of starting parameters}
\end{frame}
